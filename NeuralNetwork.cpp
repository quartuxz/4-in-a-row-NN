...
void NeuralNetwork::createSynapses(std::vector<unsigned int> layers)
{
	...
	for (size_t i = 0; i < layers.size(); i++)
	{
		...
		if (i > 0) {
			for (size_t o = 0; o < m_neurons[i-1].size(); o++)
			{
				...
				{
					m_neurons[i - 1][o].addOutGoingNeuron(std::pair<Neuron*, float>(thisLayerNeurons[p], float(range(engine)) / detail));
				}
...
void Neuron::mutateSynapses(unsigned int amountRange, unsigned int dividedBy)
{
	...
	for (size_t i = 0; i < m_outGoingNeurons.size(); i++)
	{
		m_outGoingNeurons[i].second += range(engine) / dividedBy;
	}
}
